...
public class ElegirLibros extends JFrame {
		...
		btnRealizarVenta.addActionListener(e -> {
			...
			btnConfirmarVenta.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					Exportacion venta = new Exportacion(totalVenta, fechaVenta, (String) selectorPago.getSelectedItem(),
							(String) selectorMoneda.getSelectedItem(), estado, fkTipoVenta, fkCarrito, fkUsuario,
							(String) selectorOrigen.getSelectedItem(), (String) selectorDestino.getSelectedItem(),
							estadoEnvio);
	...
	public void cargarLibro(JComboBox selector, JLabel lblError, LinkedList<CarritoDetalle> librosCarrito,
					...
					if (librosCarrito.isEmpty()) {
						for (Libro libro : listaLibros) {
							if (nombreLibro.equalsIgnoreCase(libro.getTitulo())
									&& libro.getStock() >= Integer.parseInt(textCantidad.getText())) {
						...
						// descontamos de la lista principal de libros la cantidad de libros que estan
						// comprando
						for (Libro libro : listaLibros) {
							if (nombreLibro.equalsIgnoreCase(libro.getTitulo())
									&& libro.getStock() >= Integer.parseInt(textCantidad.getText())) {
						...
						// buscamos en el carrito que ya tiene los otro libros que selecciono y le
						// agregamos el stock
						if (sinStock != true) {
							for (CarritoDetalle carrito : librosCarrito) {
								if (carrito.getFkLibro().getTitulo().equalsIgnoreCase(nombreLibro)) {
									carrito.setCantidad(
											carrito.getCantidad() + Integer.parseInt(textCantidad.getText()));
									encontrado = false;
									break;
								}
							}

							// si no encuentra un libro previamente seleccionado es un libro nuevo y lo
							// carga al carrito
							if (encontrado) {
								for (Libro libro : listaLibros) {
									if (nombreLibro.equalsIgnoreCase(libro.getTitulo())) {
										librosCarrito.add(
												new CarritoDetalle(Integer.parseInt(textCantidad.getText()), libro));
										libro.setStock(libro.getStock() - Integer.parseInt(textCantidad.getText()));
										break;
									}
								}
