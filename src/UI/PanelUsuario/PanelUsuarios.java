...
public class PanelUsuarios extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PanelUsuarios frame = new PanelUsuarios(null);
	...
	public PanelUsuarios(Usuario user) {
		...
		lblNewLabel.setBounds(249, 11, 307, 37);
		...
		model = new DefaultTableModel(
				new String[] { "ID", "USUARIO", "NOMBRE", "ROL", "ESTADO"}, 0);
		...
		JButton btnNewButton = new JButton("Nuevo Empleado");
		...
		btnNewButton.setBounds(10, 386, 156, 42);
		...
		btnDarDeBaja.setBounds(176, 386, 185, 42);
		...
		btnVolverAlMenu.setFont(new Font("Tahoma", Font.BOLD, 11));
		btnVolverAlMenu.setBounds(683, 449, 156, 42);
	...
	private void cargarTabla() {
		...
		LinkedList<Usuario> listaUsuarios = UsuarioDTO.mostrarUsuarios();
		for(Usuario usuario : listaUsuarios) {
			model.addRow(new Object[] {usuario.getId_usuario(), usuario.getUsuario(), usuario.getNombre(), usuario.getFkTipoEmpleado().getTipoEmpleado(), (usuario.getEstado() == true ? "Activo" : "Inactivo")});
		}
		}
}
