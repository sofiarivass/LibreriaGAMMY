...
public class ModificarVentaLocal extends JFrame {
	...
	public ModificarVentaLocal(Usuario user) {
		...
		model = new DefaultTableModel(new String[]{"ID-VENTA", "FECHA-VENTA", "METODO-PAGO", "Estado"}, 0);
				...
				if (selectorCliente.getSelectedItem() != "Selección") {
					...
					if (listaCarrito != null) {
						for (int i = 0; i < listaCarrito.size(); i++) {
							ventaExport = VentasLocalDTO.verVentasLocales(listaCarrito.get(i).getIdCarrito(),fkTipoVenta);
							if (ventaExport != null) {
		...
		btnVolver.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				PanelGestionarVentas gestionExport = new PanelGestionarVentas(user);
		...
		lblErrorPago.setBounds(166, 233, 156, 13);
		...
		lblErrorMoneda.setBounds(450, 228, 156, 13);
		...
		lblMetodoPago.setBounds(50, 211, 103, 21);
		panel_2.add(lblMetodoPago);
		
		JComboBox selectorPago = new JComboBox();
		selectorPago.setBounds(163, 211, 95, 21);
		...
		lblTipoMoneda.setBounds(335, 206, 103, 21);
		panel_2.add(lblTipoMoneda);
		
		JComboBox selectorMoneda = new JComboBox();
		selectorMoneda.setBounds(448, 206, 95, 21);
		...
		selectorPago.addActionListener(e -> {
			lblMensaje.setText("");
			bandera = actulizarDatos(selectorPago,selectorMoneda);
		...
		lblDatosPasados.setBounds(72, 40, 471, 113);
		...
		btnConfirmar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				boolean flagDos = VentasLocalDTO.actualizarVentaLocalJframe(venta);
	...
	public void librosElegidos(LinkedList<Venta> listaVentas) {
		
		model.setRowCount(0);
		for (Venta L : listaVentas) {
	...
	public boolean actulizarDatos(JComboBox selectorPago,JComboBox selectorMoneda) {
		boolean flag;
		
		String pago = (String) selectorPago.getSelectedItem();
		String moneda = (String) selectorMoneda.getSelectedItem();
		
		if (!pago.equals("Seleccionar") || !moneda.equals("Seleccionar")) {
