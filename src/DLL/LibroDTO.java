...
public class LibroDTO {
	...
	public static boolean agregarLibro(Libro libro) {
		...
		if (flag) {
			try {
				...
				statement.setString(11, libro.getMaterialTapa());
			...
			JOptionPane.showMessageDialog(null,
					"Ya hay un libro con las mismas carácteristicas cargado en el sistema:\n"
	...
	public static Libro libroPorID() {
		...
		for (int i = 0; i < librosArray.length; i++) {
			librosArray[i] = libros.get(i).getTitulo() + "/" + libros.get(i).getidLibro();
		}
		String elegido = (String) JOptionPane.showInputDialog(null, "Elija libro:", null, 0, null, librosArray,
				librosArray[0]);
		id_libro = Integer.parseInt(elegido.split("/")[1]);
		Libro libro = null;
		try {
			...
			if (rs.next()) {
				...
				libro = new Libro(id_libro, titulo, autor, editorial, anio_publicacion, genero, idioma, publico_objetivo, num_paginas, firmado, edicion_especial, tapa, saga, precio, stock);
			}
